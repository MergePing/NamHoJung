...
(
    ...
    FOREIGN KEY (MBTI_TYPE) REFERENCES SIZE (TBL_MBTI_INFO) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (USER_NO) REFERENCES SIZE (TBL_USER) ON UPDATE CASCADE ON DELETE CASCADE,
 PRIMARY KEY ( `MBTI_TYPE`,`MBTI_NO` )
FOR
) COMMENT = 'MBTI';

ALTER TABLE `TBL_MBTI`
 ADD CONSTRAINT `TBL_MBTI_PK` PRIMARY KEY ( `MBTI_TYPE`,`MBTI_NO` );

DROP TABLE `TBL_MBTI_INFO`;

CREATE TABLE `TBL_MBTI_INFO`
(
    `MBTI_TYPE`    VARCHAR(255) NOT NULL COMMENT 'MBTI 유형',
...
ALTER TABLE `TBL_MBTI_INFO`
 ADD CONSTRAINT `TBL_MBTI_INFO_PK` PRIMARY KEY ( `MBTI_TYPE` );
...
(
    ...
    `MBTI_QUESTION`    VARCHAR(255) NOT NULL COMMENT 'MBTI 질문'
) COMMENT = 'MBTI 테스트';
